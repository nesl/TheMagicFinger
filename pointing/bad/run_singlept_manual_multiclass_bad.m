...
% N=round(length(true_features)/2);
% TrainingSet=true_features(1:N,:);
% TestSet=    true_features(N+1:length(true_features),:);
% 
% GroupTrain=true_label(1:N);
% results = multisvm(TrainingSet, GroupTrain, TestSet);
% nCorrect = sum(results == true_label(round(length(true_features)/2):length(true_features)));
% fprintf('Accuracy: %f\n', nCorrect/length(results));


N = size(true_features,1) ;
P = cvpartition(N, 'Holdout', 0.30);
TrainingSet=true_features(P.training,:);
TestSet=    true_features(P.test,:);
% 
GroupTrain=true_label(P.training);
results = multisvm(TrainingSet, GroupTrain, TestSet);

nCorrect = sum(results == true_label(P.test) );
fprintf('Accuracy: %f\n', nCorrect/length(results));

RF = TreeBagger(100,true_features(P.training,:),true_label(P.training,:), 'Method', 'regression', 'oobpred','on');

Cvals = RF.predict(true_features(P.test,:));
nCorrect = sum( round(Cvals) == true_label(P.test) );
fprintf('Accuracy: %f\n', nCorrect/length(Cvals));
