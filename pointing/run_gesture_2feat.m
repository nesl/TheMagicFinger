...
logfolder = 'C:\Users\Alanwar\Dropbox\gesture_2feat\';
...
[peaktimes_z_p,  peaktimes_x_p,  locs_z_p,locs_x_p] = getGestureEvents_2feat(iot);

numofpoints = length(peaktimes_z_p) + length(peaktimes_x_p) ;
SRCID = 9;
WLEN = 0.6;
...
P = cvpartition(N, 'Holdout', 0.60);
svmStruct = svmtrain( all_features(P.training,:), all_labels(P.training,:), 'kernel_function', 'linear');
...
accuracy = n_truepos +n_trueneg/ (n_truepos +n_trueneg+ n_falseneg+n_falsepos);
fprintf('===== SVM =====\n');
