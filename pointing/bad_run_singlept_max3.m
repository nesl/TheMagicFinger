...
for i=1:length(point_idxs)
    ...
    for j=1:length(iot.nodes)
        ...
        Cs = [Cs; c];
        Aids = [Aids; n.id];
        position = [position , n.getTruePosition];
    end
    Cvals = Cs;
    % collaborative guess
   if(max(Cvals) ==1)
        if(length(Aids) >2)
            ...
            [~,idxs] = sort(Cvals,'descend'); %Cs
            postion1= position(:,idxs(1));
            postion2= position(:,idxs(2));
            postion3= position(:,idxs(3));
            
            pos_x = [ postion1(1) postion2(1) postion3(1)];
            pos_z = [ postion1(3) postion2(3) postion3(3)];
            
            if var(pos_x) > var(pos_z)
                [~,indexs]=sort(pos_x,'descend');
            else
                [~,indexs]=sort(pos_x,'descend');
            end
            aid_guess = Aids(indexs(2));
            aid_guesses = [aid_guesses; aid_guess];
        elseif (length(Aids)==2)
            [~,idxs] = sort(Cvals,'descend'); %Cs
            postion1= position(:,idxs(1));
            postion2= position(:,idxs(2));
            
            pos_x = [ postion1(1) postion2(1) ];
            pos_z = [ postion1(3) postion2(3) ];
